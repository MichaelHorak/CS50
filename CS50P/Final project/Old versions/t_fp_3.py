    ...
    for artist in artist_ids:
        ...
        for result in o["results"]:
            try:
                ...
                # date = result["releaseDate"]
                release_date = date[:4]
                insert_into_db(artist_name, collection_name, track_name,
                               release_date)
            except KeyError:
...
def insert_into_db(artist_name, collection_name, track_name, release_date):
    ...
    cur = con.cursor()
    cur.execute(
        "CREATE TABLE IF NOT EXISTS songdata(artist_name TEXT, collection_name TEXT, track_name TEXT, release_date"
        " INTEGER)")
    # insert data into the database
    cur.execute("INSERT INTO songdata VALUES(?, ?, ?, ?)",
                (artist_name, collection_name, track_name, release_date))
    # SELECT ALL
    res = cur.execute("SELECT COUNT(release_date) AS year FROM songdata GROUP BY collection_name ORDER BY year DESC LIMIT 1")
    # SELECT
    #     s.release_date,
    #     COUNT(s.release_date) AS year
    # FROM music_cs50p.songdata s
