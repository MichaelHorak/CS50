            ...
            for result in self.o["results"]:
                try:
                    ...
                    self.unwanted_albums = ['special edition', 'deluxe edition', 'single', 'soundtrack',
                                            'motion picture', 'collection', 'remastered', 'remaster', 'mix',
                                            'expanded edition']
                    # self.insert_into_db()
                    # Check if any unwanted keyword is in collection_name
                    if not any(keyword in self.collection_name.lower() for keyword in self.unwanted_albums):
                        if self.artist_name in genres[(int(self.genre_choice) - 1)]:
                            self.insert_into_db()
                except KeyError:
